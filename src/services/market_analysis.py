...
class MarketAnalyzer:
    ...
    def __init__(self, api_key: Optional[str] = None):
        ...
        try:
            ...
            if api_key is None:
                raise ValueError(
                    "未找到DeepSeek API密钥，请设置DEEPSEEK_API_KEY环境变量或直接传入api_key参数"
                )

            # 初始化DeepSeek聊天模型
            self.llm = DeepSeekChatModel(
                api_key=api_key, temperature=0.7, max_tokens=2048
            )
...
请生成一份详细的分析报告。请注意：不要使用Markdown格式（如#、##、-、*等标记）输出内容，请使用纯文本格式，以便于PDF导出。对于标题和重点内容，可以使用空行分隔或直接描述"标题："等方式表达层次结构。""",
        )

        # 设置波动率预测模板
        self.volatility_prediction_template = PromptTemplate(
            input_variables=[
                "token_symbol",
                "current_volatility",
            ...
            template="""请基于以下数据对{token_symbol}未来{horizon}天的波动率进行深度分析和预测：
...
2. 可能的波动率峰值和谷值
3. 影响波动率的关键因素分析
4. 不同情景下的波动率预测（乐观/中性/悲观）
5. 风险评估和建议

请提供详细的分析和具体的数值预测。请注意：不要使用Markdown格式输出内容，请使用纯文本格式，以便于PDF导出。""",
        )

        # 创建报告生成链
        self.report_chain = LLMChain(llm=self.llm, prompt=self.report_template)

        # 创建波动率预测链
        self.volatility_prediction_chain = LLMChain(
            llm=self.llm, prompt=self.volatility_prediction_template
        )
    ...
    def analyze_technical_indicators(self, price_data: pd.DataFrame) -> Dict[str, Any]:
        ...
        return {
            ...
            "rsi": rsi.iloc[-1],
            "macd": {"macd": macd.iloc[-1], "signal": signal.iloc[-1]},
        }

    def analyze_correlations(
        self, token_price: pd.Series, comparison_assets: Dict[str, pd.Series]
    ) -> Dict[str, float]:
        """
        分析与其他资产的相关性
    ...
    def generate_market_analysis(
        ...
        comparison_assets: Dict[str, pd.Series] = None,
    ) -> str:
        """
        生成综合市场分析报告
        ...
        if comparison_assets:
            correlation_data = self.analyze_correlations(
                price_data["price"], comparison_assets
            )
        ...
        market_data = {
            ...
            "24h_change": (
                price_data["price"].iloc[-1] / price_data["price"].iloc[-24] - 1
            )
            * 100,
        ...
        volatility_summary = {
            ...
            "30d_avg": (
                volatility_data.iloc[-30:].mean()
                if len(volatility_data) >= 30
                else None
            ),
            "trend": (
                "上升"
                if volatility_data.iloc[-1] > volatility_data.iloc[-7:].mean()
                else "下降"
            ),
        }
        ...
        report = self.report_chain.run(
            {
                ...
                "technical_indicators": json.dumps(
                    technical_indicators, ensure_ascii=False, indent=2
                ),
                "correlation_data": json.dumps(
                    correlation_data, ensure_ascii=False, indent=2
                ),
                "volatility_data": json.dumps(
                    volatility_summary, ensure_ascii=False, indent=2
                ),
    ...
    def predict_volatility_with_deepseek(
        self,
        token_symbol: str,
        price_data: pd.DataFrame,
        volatility_data: pd.Series,
        ...
        """
        使用DeepSeek API进行波动率预测分析

        Args:
            token_symbol: 代币符号
            price_data: 价格数据
            volatility_data: 波动率数据
            horizon: 预测时间范围（天数），默认7天
        ...
        price_trend = {
            "current_price": price_data["price"].iloc[-1],
            "7d_change": (
                (price_data["price"].iloc[-1] / price_data["price"].iloc[-7] - 1) * 100
                if len(price_data) >= 7
                else None
            ),
            "30d_change": (
                (price_data["price"].iloc[-1] / price_data["price"].iloc[-30] - 1) * 100
                if len(price_data) >= 30
                else None
            ),
            "trend": (
                "上升"
                if price_data["price"].iloc[-1] > price_data["price"].iloc[-7:].mean()
                else "下降"
            ),
        }
        ...
        market_data = {
            "current_price": price_data["price"].iloc[-1],
            "24h_change": (
                (price_data["price"].iloc[-1] / price_data["price"].iloc[-24] - 1) * 100
                if len(price_data) >= 24
                else None
            ),
            "24h_volume": (
                price_data["volume"].iloc[-24:].sum() if len(price_data) >= 24 else None
            ),
            "7d_high": (
                price_data["price"].iloc[-7:].max() if len(price_data) >= 7 else None
            ),
            "7d_low": (
                price_data["price"].iloc[-7:].min() if len(price_data) >= 7 else None
            ),
        }

        # 准备当前波动率数据
        current_volatility = {
            "value": volatility_data.iloc[-1],
            "percentile": (
                np.percentile(
                    [i for i in range(100)],
                    volatility_data.iloc[-1] / volatility_data.max() * 100,
                )
        ...
        # 准备历史波动率数据
        historical_volatility = {
            "mean": volatility_data.mean(),
            "median": volatility_data.median(),
            "max": volatility_data.max(),
            "min": volatility_data.min(),
            "std": volatility_data.std(),
            "recent_trend": (
                "上升"
                if volatility_data.iloc[-1] > volatility_data.iloc[-7:].mean()
                else "下降"
        ...
        # 使用DeepSeek进行波动率预测分析
        prediction_report = self.volatility_prediction_chain.run(
            {
                "token_symbol": token_symbol,
                "current_volatility": json.dumps(
                    current_volatility, ensure_ascii=False, indent=2
                ),
                "historical_volatility": json.dumps(
                    historical_volatility, ensure_ascii=False, indent=2
                ),
                "price_trend": json.dumps(price_trend, ensure_ascii=False, indent=2),
                "market_data": json.dumps(market_data, ensure_ascii=False, indent=2),
                "technical_indicators": json.dumps(
                    technical_indicators, ensure_ascii=False, indent=2
                ),
